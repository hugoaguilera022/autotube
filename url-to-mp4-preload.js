  ...
  for(const base of bases){
    try{
      ...
      try{
        ...
        const ct=String(res.headers.get('content-type')||'').toLowerCase();
        if(!res.body)throw new Error('YT5S proxy no body');
        const out=path.join(dir,'source.mp4'),fh=await fs.open(out,'w');
        try{const reader=res.body.getReader();while(true){const part=await reader.read();if(part.done)break;await fh.write(part.value)}}finally{await fh.close()}
        const st=await fs.stat(out);
        if(!st.size||(!ct.includes('video')&&!ct.includes('octet-stream')&&!ct.includes('mp4')))throw new Error('YT5S no MP4: '+ct);
...
async function downloadViaYtdlApi(url,dir){
  const bases=['https://najemi.cz/ytdl/handler.php','https://ytdl.ga/handler.php'];
  let last='';
  for(const base of bases){
    try{
      const endpoint=base+'?url='+encodeURIComponent(url);
      const res=await fetch(endpoint,{redirect:'follow',headers:{'User-Agent':'Mozilla/5.0','Accept':'text/plain,application/json,*/*'}});
      if(!res.ok)throw new Error('YTDL API HTTP '+res.status);
      const text=await res.text();
      const urls=[...text.matchAll(/https?:\/\/[^\s"'<>]+/g)].map(m=>m[0].replace(/\\u0026/g,'&')).filter(x=>/\.mp4|googlevideo|videoplayback/i.test(x));
      if(!urls.length)throw new Error('YTDL API no devolvió enlace MP4');
      for(const media of urls){
        try{
          const fr=await fetch(media,{redirect:'follow',headers:{'User-Agent':'Mozilla/5.0','Referer':'https://www.youtube.com/'}});
          const ct=String(fr.headers.get('content-type')||'').toLowerCase();
          if(!fr.ok||!fr.body||(!ct.includes('video')&&!ct.includes('mp4')&&!ct.includes('octet-stream')))throw new Error('media HTTP '+fr.status+' '+ct);
          const out=path.join(dir,'source.mp4'),fh=await fs.open(out,'w');
          try{const reader=fr.body.getReader();while(true){const part=await reader.read();if(part.done)break;await fh.write(part.value)}}finally{await fh.close()}
          const st=await fs.stat(out);if(!st.size)throw new Error('MP4 vacío');
          const p=await probe(out);if(!p.duration||!p.width||!p.height)throw new Error('MP4 no válido');
          return{source:out,bytes:st.size,strategy:'ytdl-api'};
        }catch(e){last=String(e?.message||e);await fs.rm(path.join(dir,'source.mp4'),{force:true}).catch(()=>{})}
      }
    }catch(e){last=String(e?.message||e)}
  }
  throw new Error('YTDL API falló: '+last);
}

async function downloadViaLegacyYtdl(url,dir){
  const targets=[
    'https://najemi.cz/ytdl/handler.php?url='+encodeURIComponent(url),
    'https://ytdl.ga/handler.php?url='+encodeURIComponent(url)
  ];
  let last='';
  for(const target of targets){
    for(const endpoint of [target,'https://api.allorigins.win/raw?url='+encodeURIComponent(target)]){
      try{
        const res=await fetch(endpoint,{redirect:'follow',headers:{'User-Agent':'Mozilla/5.0','Accept':'video/mp4,application/octet-stream,text/html,*/*'}});
        if(!res.ok)throw new Error('Legacy YTDL HTTP '+res.status);
        const ct=String(res.headers.get('content-type')||'').toLowerCase();
        if(!res.body)throw new Error('Legacy YTDL sin body');
        const out=path.join(dir,'source.mp4'),fh=await fs.open(out,'w');
        try{const reader=res.body.getReader();while(true){const part=await reader.read();if(part.done)break;await fh.write(part.value)}}finally{await fh.close()}
        const st=await fs.stat(out);
        if(!st.size||ct.includes('text/html')||ct.includes('application/json')){await fs.rm(out,{force:true});throw new Error('Legacy YTDL no devolvió MP4 ('+ct+').')}
        return{source:out,bytes:st.size,strategy:'legacy-ytdl'};
      }catch(e){last=String(e?.message||e);await fs.rm(path.join(dir,'source.mp4'),{force:true}).catch(()=>{})}
    }
  }
  throw new Error(last||'Legacy YTDL failed');
}

async function downloadExactYoutube(url,dir){await fs.mkdir(dir,{recursive:true});const parsed=new URL(url);try{return await downloadViaAllDL(url,dir)}catch(e){console.error('AUTOTUBE ALLDL FAILED',e?.message||e)}try{return await downloadViaYtdlApi(url,dir)}catch(e){console.error('AUTOTUBE YTDL API FAILED',e?.message||e)}if(!/youtube\.com$|youtu\.be$/i.test(parsed.hostname)){const ext=(path.extname(parsed.pathname).toLowerCase()||'.mp4');const out=path.join(dir,'source'+ext);const r=await fetch(url);if(!r.ok)throw new Error('La URL directa devolvió HTTP '+r.status+'.');const file=await fs.open(out,'w');try{if(!r.body)throw new Error('La URL directa no devolvió contenido.');const reader=r.body.getReader();while(true){const {done,value}=await reader.read();if(done)break;await file.write(value)}}finally{await file.close()}const st=await fs.stat(out);if(!st.size)throw new Error('La URL directa devolvió un archivo vacío.');return{source:out,bytes:st.size,strategy:'direct-media-url'}}try{return await downloadViaAllDL(url,dir)}catch(e){console.error('AUTOTUBE ALLDL FAILED',e?.message||e)}
  try{return await downloadViaVevioz(url,dir)}catch(e){console.error('AUTOTUBE VEVIOZ FAILED',e?.message||e)}try{return await downloadViaLegacyYtdl(url,dir)}catch(e){console.error('AUTOTUBE LEGACY YTDL FAILED',e?.message||e)}try{return await downloadViaYt5sProxy(url,dir)}catch(e){console.error('AUTOTUBE YT5S PROXY FAILED',e?.message||e)}try{return await downloadViaCobalt(url,dir)}catch(e){console.log('AUTOTUBE COBALT FAILED',String(e?.message||e));}const out=path.join(dir,'source.%(ext)s'),potScript=path.join(process.cwd(),'.pot-provider','server','build','generate_once.js'),potArgs=`youtubepot-bgutilhttp:base_url=http://127.0.0.1:4416;youtube:player_client=mweb,web_safari`,strategies=[{name:'web-embedded',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=web_embedded,default'},{name:'android-vr',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=android_vr'},{name:'tv',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=tv'}, {name:'mweb-pot',format:'bestvideo[vcodec^=avc1][ext=mp4]+bestaudio[acodec^=mp4a][ext=m4a]/best[ext=mp4]',extractorArgs:'youtubepot-bgutilhttp:base_url=http://127.0.0.1:4416;youtube:player_client=mweb,web_safari'},{name:'web-default',format:'bestvideo[vcodec^=avc1][ext=mp4]+bestaudio[acodec^=mp4a][ext=m4a]/best[ext=mp4]',extractorArgs:'youtube:player_client=web,web_safari,android_vr,tv'},{name:'safari-hls-fallback',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=web_safari,android_vr,tv'},{name:'android-vr-fallback',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=android_vr,tv,web_embedded'},{name:'ios-android-fallback',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=ios,android,android_vr'},{name:'tv-fallback',format:'bestvideo*+bestaudio/best',extractorArgs:'youtube:player_client=tv,web_embedded,android_vr'},{name:'best-single',format:'best',extractorArgs:'youtube:player_client=web_safari,android_vr,tv'}];let last='';for(const s of strategies){try{const result=await youtubedl(url,{format:s.format,output:out,mergeOutputFormat:'mp4',noPlaylist:true,noWarnings:true,noCheckCertificates:true,restrictFilenames:true,preferFreeFormats:false,ffmpegLocation:path.dirname(ffmpegPath),retries:5,fragmentRetries:5,concurrentFragments:1,extractorArgs:s.extractorArgs,jsRuntimes:'node',remoteComponents:'ejs:github',forceIpv4:true,referer:'https://www.youtube.com/',userAgent:'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/140.0.0.0 Safari/537.36',verbose:true},{timeout:30000,killSignal:'SIGKILL'});const files=await fs.readdir(dir),c=files.filter(x=>/^source\.(mp4|mkv|webm|mov|m4v)$/i.test(x));if(!c.length)throw new Error('yt-dlp no produjo un archivo de vídeo.');const source=path.join(dir,c[0]),st=await fs.stat(source);if(!st.size)throw new Error('El archivo descargado está vacío.');return{source,bytes:st.size,strategy:s.name,ytDlpOutput:String(result||'').slice(-1500)}}catch(e){last=s.name+': '+String(e?.stderr||e?.message||e||'').slice(-6000);console.error('AUTOTUBE YTDLP STRATEGY FAILED',s.name,last);for(const f of await fs.readdir(dir).catch(()=>[]))if(/^source\./i.test(f))await fs.rm(path.join(dir,f),{force:true}).catch(()=>{})}}try{return await downloadViaPiped(url,dir)}catch(e){last='piped: '+String(e?.message||e)+' | prior='+last}try{return await downloadViaPiped(url,dir)}catch(e){last='piped: '+String(e?.message||e)+' | prior='+last}try{return await downloadViaPiped(url,dir)}catch(e){last='piped: '+String(e?.message||e)+' | prior='+last}try{return await downloadViaCobalt(url,dir)}catch(e){last='cobalt: '+String(e?.message||e)+' | prior='+last}try{return await downloadViaInvidious(url,dir)}catch(e){last='invidious: '+String(e?.message||e)+' | prior='+last}if(process.env.AUTOTUBE_EXTERNAL_DOWNLOADER_URL){try{return await downloadViaExternalProvider(url,dir)}catch(e){last='external-provider: '+String(e?.message||e)}}throw new Error('No se pudo obtener el vídeo de referencia por ninguna fuente configurada. Último error: '+last)}
