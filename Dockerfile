...
RUN sed -i 's/await self.sleep(0.5)/await self.sleep(2)/' /usr/local/lib/python3.12/site-packages/nodriver/core/browser.py && sed -i 's/nodriver.start(headless=False,/nodriver.start(headless=False, no_sandbox=True,/' /app/session/potoken_generator/extractor.py
EXPOSE 8080
