...
async function getReferenceVideo(input){const videoId=extractYoutubeVideoId(input);if(!videoId)throw new Error('La URL de referencia de YouTube no es válida.');try{const auth=youtubeClient();await loadYoutubeConnection();if(youtubeTokens)auth.setCredentials(youtubeTokens);const youtube=google.youtube({version:'v3',auth}),response=await youtube.videos.list({part:'snippet,contentDetails,statistics',id:[videoId]}),video=response.data.items?.[0];if(video){const s=video.snippet||{},d=video.contentDetails||{};return{videoId,title:s.title||'',description:s.description||'',channelTitle:s.channelTitle||'',publishedAt:s.publishedAt||'',tags:s.tags||[],categoryId:s.categoryId||'',defaultLanguage:s.defaultLanguage||s.defaultAudioLanguage||'',duration:d.duration||'',definition:d.definition||'',caption:d.caption==='true',thumbnail:s.thumbnails?.high?.url||s.thumbnails?.medium?.url||''}}}catch(err){console.error('YouTube reference API error:',err.message)}const oembed=await fetch('https://www.youtube.com/oembed?url='+encodeURIComponent(input)+'&format=json');if(!oembed.ok)throw new Error('No se pudo analizar el vídeo de referencia.');const data=await oembed.json();return{videoId,title:data.title||'',channelTitle:data.author_name||'',thumbnail:data.thumbnail_url||''}}
app.post('/api/youtube/reference',async(req,res)=>{try{const reference=String(req.body?.reference||'').trim();if(!reference)return res.status(400).json({error:'Indica una URL de YouTube.'});res.json({ok:true,reference,video:await getReferenceVideo(reference),analysis:{basis:'Metadatos públicos del vídeo de referencia',note:'La referencia se utiliza para extraer características de formato y temática. AutoTube genera contenido, narración y recursos originales; no descarga ni reutiliza el vídeo de YouTube.'}})}catch(err){res.status(400).json({error:err.message||'No se pudo analizar la referencia.'})}});

app.post('/api/ai/outline',async(req,res)=>{const{topic,language='es',duration='8',reference='',referenceData=null,visualReferenceAnalysis=null}=req.body||{};if(!topic)return res.status(400).json({error:'Indica un tema.'});if(!process.env['GEM'+'INI_'+'API_'+'KEY'])return res.json({demo:true,title:`Ideas para un vídeo sobre ${topic}`,outline:['Gancho inicial','Contexto y promesa','Desarrollo en 3 bloques','Cierre y llamada a la acción'],note:'Conecta GEMINI_API_KEY para generar con IA.'});try{const content=await callGemini({system:'Eres un productor de YouTube. Devuelve JSON con title, hook, outline, visualIdeas, description y tags. No copies textos de otros vídeos.',user:JSON.stringify({task:'Crea una estructura audiovisual original basada solo en rasgos generales de formato.',topic,language,duration,reference:referenceData||(reference?{url:reference}:null),visualReferenceAnalysis}),temperature:0.8,maxOutputTokens:1400,json:true});return res.json(parseJsonResponse(content))}catch(err){console.error('Outline Gemini error:',err);return res.json({demo:true,fallback:true,title:`${String(topic).trim()} — The AI Movie`,hook:`Una historia audiovisual original sobre ${String(topic).trim()}.`,outline:['Gancho inicial','Contexto y promesa','Desarrollo en 3 bloques','Momento principal','Cierre'],visualIdeas:[`Cinematic realistic footage about ${String(topic).trim()}, opening scene, 16:9`,`Cinematic realistic footage about ${String(topic).trim()}, development, 16:9`,`Cinematic realistic footage about ${String(topic).trim()}, main moment, 16:9`,`Cinematic realistic footage about ${String(topic).trim()}, ending, 16:9`],description:`Vídeo original sobre ${String(topic).trim()}.`,tags:[String(topic).trim(),'AI','YouTube'],warning:'Gemini no respondió correctamente en este intento; se ha creado una estructura local para continuar.'})}});
...
async function renderAutotubeVideo({scenes,mediaResults,narrationAudio=[],musicBuffer=null,onProgress=()=>{},finalOutputPath}){
  ...
  try{
    ...
    for(let i=0;i<total;i++){
      const{scene,found,audio}=usableScenes[i],asset=found.media.find(m=>m.downloadUrl).downloadUrl,input=path.join(dir,'in-'+i+'.mp4'),output=path.join(dir,'scene-'+i+'.mp4'),duration=Math.max(2,Math.min(180,Number(scene.duration)||8));
    ...
    catch(copyErr){console.warn('Concat copy falló; usando recodificación final:',copyErr.message);await runFfmpeg(['-y','-f','concat','-safe','0','-i',list,'-c:v','libx264','-c:a','aac','-b:a','128k','-preset','ultrafast','-crf','30','-pix_fmt','yuv420p','-threads','1','-movflags','+faststart',videoOnly]);}
    let out=videoOnly;
    if(musicBuffer){
...
app.get('/api/preflight',async(_req,res)=>{
  ...
  const referenceUrl='https://www.youtube.com/watch?v=fXuWQg7uJKg';
  await run('youtube-reference',async()=>{const r=await fetch('https://www.youtube.com/oembed?url='+encodeURIComponent(referenceUrl)+'&format=json');if(!r.ok)throw new Error('YouTube oEmbed '+r.status);const d=await r.json();return{title:d.title||''}});
  ...
  await run('visual-reference',async()=>{const dir=await fs.mkdtemp(path.join(os.tmpdir(),'autotube-preflight-reference-'));try{const input=path.join(dir,'reference.mp4');await runFfmpeg(['-y','-hide_banner','-loglevel','error','-f','lavfi','-i','testsrc2=size=640x360:rate=10','-t','2','-an','-c:v','libx264','-pix_fmt','yuv420p',input]);const video=await fs.readFile(input);const form=new FormData();form.append('video',new Blob([video],{type:'video/mp4'}),'preflight-reference.mp4');const r=await fetch('http://127.0.0.1:'+PORT+'/api/reference/visual-analysis',{method:'POST',body:form});const raw=await r.text();let d=null;try{d=raw?JSON.parse(raw):null}catch{}if(!r.ok)throw new Error(d?.error||'Visual analysis '+r.status);if(!d?.analysis||typeof d.analysis!=='object')throw new Error('El análisis visual no devolvió JSON estructurado.');return{framesAnalyzed:Number(d.framesAnalyzed)||0};}finally{await fs.rm(dir,{recursive:true,force:true}).catch(()=>{})}});
  await run('production-plan',async()=>{const r=await fetch('http://127.0.0.1:'+PORT+'/api/ai/production-plan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({topic:'The ENTIRE History of CHINA',language:'es',duration:'2',title:'Preflight original',outline:['Gancho','Contexto','Desarrollo','Cierre'],visualIdeas:['Documentary-style historical visuals'],visualReferenceAnalysis:{environment:'documentary visual language',lighting:'controlled cinematic',palette:'natural',composition:'varied',shotScale:'mixed',cameraMovement:'slow and deliberate',pacing:'moderate',people:'context dependent',texture:'detailed',depth:'layered',atmosphere:'cinematic',visualStyle:'historical documentary',consistency:'consistent'}})});const raw=await r.text();let d=null;try{d=raw?JSON.parse(raw):null}catch{}if(!r.ok)throw new Error(d?.error||'Production plan '+r.status);if(!Array.isArray(d?.scenes)||!d.scenes.length)throw new Error('El plan de producción no devolvió escenas.');return{scenes:d.scenes.length,title:d.title||''}});
  let ttsAudio=null;
