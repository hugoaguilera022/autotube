...
app.post('/api/ai/production-plan',async(req,res)=>{try{
  const {
    ...
    referenceData=null,reference='',transcript='',referenceBlueprint=null
  }=req.body||{};
  ...
  const referenceContext={
    ...
    transcript:String(transcript||referenceData?.transcript||'').slice(0,100000),
  ...
  const system='Eres director de producción audiovisual de YouTube. Debes generar escenas para un vídeo ORIGINAL basado en una referencia de YouTube. La referencia es la fuente principal del TEMA, TIPO DE CONTENIDO y LENGUAJE AUDIOVISUAL. No inventes un tema distinto. REGLA DE GUIÓN: por defecto SIEMPRE debes escribir un guion/narración NUEVO y ORIGINAL. Aunque exista transcript o audio de la referencia, úsalo únicamente para comprender el tema, hechos, estructura y secuencia; NO copies ni reproduzcas literalmente su guion. SOLO cuando customScript contenga texto escrito por el usuario debes usar ese texto como guion proporcionado por el usuario. Si customScript está vacío, la narración de TODAS las escenas debe ser nueva, redactada por la IA y coherente con el tema de referencia. Si el usuario no escribió un tema explícito, el asunto DEBE permanecer alineado con referenceContext.title, description, tags y videoProfile. No conviertas automáticamente ningún vídeo en naturaleza, relajación, bienestar, documentales genéricos ni Fosa de las Marianas. Usa el contenido real de la referencia y su análisis para decidir qué aparece en las escenas. Las visualReferences del usuario son referencias creativas prioritarias y deben traducirse a características generales de estilo. El estilo audiovisual se reutiliza solo como características generales de composición, paleta, iluminación, ritmo, movimiento, animación, locución, relación voz/música y tratamiento sonoro; nunca planos, textos, personajes, guion literal, melodías ni contenido identificable. Devuelve JSON válido con title, musicMood, voiceStyle y scenes. Cada escena debe tener number,title,narration,visualPrompt,searchQuery,duration,transition,mediaType,constantImage y animationNotes. Si useSingleContinuousVisual=true o constantImage=true, genera EXACTAMENTE UNA escena con toda la duración, mediaType=image, constantImage=true y una searchQuery que busque una única imagen horizontal coherente con el TEMA REAL de la referencia. No inventes cambios de escena. Si no es imagen constante, crea escenas cuyo contenido siga estrictamente el tema y la estructura de la referencia. Si referenceContext.referenceBlueprint existe, es el MAPA TEMPORAL Y AUDIOVISUAL PRIORITARIO: respeta su número de secciones, startRatio/endRatio, propósito, sujeto visual, tipo de plano, movimiento de cámara, composición, intensidad de movimiento, transición y papel del audio. Después usa referenceContext.structureProfile.sceneSegments como detalle adicional. asigna cada escena al tramo temporal correspondiente y utiliza sus subject, shotScale, composition, cameraMovement, motionIntensity, lighting, palette, transitionIn/Out, audioRole, narrationRole y continuityAnchor para que cada escena tenga instrucciones audiovisuales específicas. Añade a cada escena un campo referenceSegment con startSeconds,endSeconds,summary,generationPrompt y continuityAnchor cuando exista un segmento correspondiente. No mezcles características de segmentos lejanos si eso rompe la continuidad. En cada visualPrompt y searchQuery incluye el sujeto/tema concreto derivado de la referencia, además de los rasgos visuales generales. Antes de responder comprueba internamente que ninguna escena se ha desviado del tema principal y que la secuencia temporal completa queda cubierta sin saltos injustificados. Crea contenido original.';

  const userPayload={
    task:'Crear el plan de escenas del vídeo nuevo manteniendo el tema y las instrucciones del usuario, pero con contenido original. Si transcript contiene la transcripción del vídeo de referencia, úsala como fuente principal para comprender el contenido y la secuencia narrativa. No copies literalmente frases; genera narración original.',
    effectiveTopic,brief,visualReferences,referenceTopic,referenceContext,referenceSceneSegments:refStructure?.sceneSegments||[],language,duration,title,outline,visualIdeas,
    visualReferenceAnalysis,referenceStyle,sceneCount:requestedSceneCount,referenceAudioProfile:refAudio,referenceAnimationProfile:refProfile?.animationProfile||{},referenceStructureProfile:refStructure,referenceBlueprint:referenceBlueprint||null,
    hardRules:{
      themeAnchor:referenceContext.title,
      referenceBlueprintRequired:Boolean(referenceBlueprint?.sections?.length),
      preserveReferenceTiming:true,
      preserveReferenceShotLanguage:true,
      singleVisual,
...
app.get('/video-ai-test-browser',async(req,res)=>{res.type('html').send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>AutoTube · LTX batch</title></head><body style="font-family:system-ui;max-width:900px;margin:30px auto;padding:20px"><h1>AutoTube · prueba real LTX</h1><p id="status">Preparando…</p><pre id="log"></pre><script type="module">
import{Client}from"https://cdn.jsdelivr.net/npm/@gradio/client/dist/index.min.js";
const batch=Math.max(0,Math.min(5,Number(new URLSearchParams(location.search).get("batch")||0)));
const groups=[
["a football floating above a luminous ocean cliff at sunrise, an original unbranded ball, sweeping aerial camera, cinematic photorealism","a surreal football drifting above a vast coastal landscape, golden sunrise, realistic reflections, slow cinematic push-in","an original footballer approaching a glowing ball on a cliff above the sea, dramatic backlight, wide cinematic framing","a football rolling across wet volcanic rock beside the ocean, mist and sunlight, low tracking camera","an original player silhouetted against a brilliant ocean horizon, floating islands in the distance, epic sports film","a luminous football crossing a reflective shoreline at dawn, atmospheric haze, dynamic cinematic camera"],
["an original crystalline football inspired by Brazilian colors resting on warm desert sand, photorealistic macro cinematography","a translucent faceted football with green and yellow reflections moving across desert dunes, cinematic close-up","an original unbranded football made of glass-like facets reflecting a tropical landscape, dramatic sunlight","a crystalline football casting colorful reflections on sand, shallow depth of field, smooth camera orbit","an original footballer reaching toward a glowing faceted ball in a surreal desert, cinematic wide shot","a jewel-like football spinning above warm sand with realistic shadows and lens flare, high-end commercial film"],
["an original footballer in a red fictional kit standing in shallow water beneath floating islands and waterfalls, epic fantasy sports cinema","a fictional player running through reflective water with floating mountains and waterfalls, sunrise haze, tracking shot","an original red-kit footballer facing a surreal sky filled with floating islands, dramatic backlight and mist","a footballer splashing through shallow water toward a glowing ball, floating landscapes behind, dynamic low angle","an original athlete surrounded by enormous clouds and suspended islands, realistic water motion, cinematic camera push","a fictional stadium-like landscape transformed into a flooded dream world, football action, photorealistic"],
["two original unbranded footballers walking through a bright cinematic landscape, fictional blue and white kits, heroic framing","an original football captain in a blue fictional kit walking beside a teammate, strong sunlight, shallow depth of field","two fictional players crossing a surreal open landscape with mountains and clouds, natural human motion","an original footballer standing against a huge sunlit sky, blue and white colors, cinematic portrait","a fictional team of footballers moving toward a distant stadium-like horizon, dramatic wide composition","an original player looking across a fantastical landscape with floating clouds, realistic sports documentary lighting"],
["an original footballer in orange training gear controlling a ball on a city street, dynamic low camera, photorealistic sports commercial","close-up of an original football boot striking a fictional ball on wet pavement, fast tracking camera, dramatic reflections","an unbranded football rolling toward an original player in a modern city, cinematic shallow depth of field","an original athlete dribbling through a rain-soaked urban environment, dynamic handheld sports cinematography","a fictional footballer accelerating beside bicycles and modern architecture, realistic motion blur and sunlight","an original close-up of a football and boots moving through a futuristic city plaza, high-end commercial cinematography"],
["an original footballer in a fictional pink kit surrounded by floating footballs and fragments of a surreal stadium, cinematic action","a fictional player controlling a glowing ball amid floating debris and clouds, dramatic low angle, epic lighting","an original goalkeeper-like athlete reaching toward a luminous football in a surreal volcanic landscape, cinematic depth","a fictional footballer in a blue kit with a glowing ball and distant mountains, dramatic atmospheric perspective","an original player sprinting toward camera through a fantastical stadium landscape, realistic cloth and body motion","a luminous football flying toward an original unbranded athlete with a volcano and floating terrain behind, cinematic finale"]
];
const base="Photorealistic high-end AI football movie, 16:9, dramatic cinematic lighting, realistic human anatomy and motion, atmospheric haze, detailed textures, dynamic camera movement, coherent continuity, premium sports-film cinematography. ORIGINAL CONTENT ONLY: no real players, no real teams, no logos, no numbers, no text, no watermark, no copied shot or frame.";
const prompts=groups[batch].map((x,i)=>x+". "+base+" Scene "+(batch*6+i+1)+" of a longer original football film; vary the composition and camera movement while preserving the same audiovisual language.");
const negative="worst quality, inconsistent motion, blurry, jittery, distorted anatomy, duplicate limbs, text, logos, watermark, real person likeness";
const s=document.getElementById("status"),l=document.getElementById("log"); window.__outputs=[];
(async()=>{try{const client=await Client.connect("Lightricks/ltx-video-distilled");for(let i=0;i<prompts.length;i++){s.textContent="Generando clip "+(i+1)+"/"+prompts.length+" · lote "+(batch+1)+"/6";const r=await client.predict("/text_to_video",[prompts[i],negative,null,null,288,512,"text-to-video",8.5,9,Math.floor(Math.random()*4294967295),true,3,true]);const o=r?.data?.[0],u=typeof o==="string"?o:(o?.url||o?.path||o?.video?.url||"");if(!u)throw Error("LTX no devolvió el clip "+(i+1));window.__outputs.push(u);l.textContent=JSON.stringify({batch,completed:window.__outputs.length,total:prompts.length});}s.textContent="DONE";window.__done=true}catch(e){s.textContent="ERROR";l.textContent=e?.stack||String(e);window.__error=String(e)}})();
</script></body></html>`)});
