  ...
  for(const model of models){
    const body={
      ...
      input:[
        ...
        {type:'video',uri:referenceUrl,processing:'agentic'}
      ]
    };
...
app.get('/video-ai-test-browser',async(_req,res)=>{res.type('html').send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AutoTube · Prueba vídeo IA</title><style>body{font-family:system-ui,sans-serif;background:#0b0d12;color:#f3f4f6;display:grid;place-items:center;min-height:100vh;margin:0}.card{max-width:680px;margin:24px;padding:28px;border:1px solid #252936;border-radius:18px;background:#12151c}button{padding:12px 18px;border:0;border-radius:10px;background:#fff;color:#111;font-weight:700;cursor:pointer}pre{white-space:pre-wrap;background:#090b0f;padding:14px;border-radius:10px;color:#cbd5e1}video{width:100%;margin-top:16px;border-radius:12px}</style></head><body><main class="card"><h1>Prueba real de vídeo IA</h1><p>La generación se ejecuta directamente desde este navegador en Hugging Face ZeroGPU, no dentro de Render. Así evitamos que el servidor se reinicie por la carga de IA.</p><button id="go">Generar clip de prueba (3 s)</button><p id="status">Esperando…</p><pre id="log"></pre><video id="video" controls hidden></video></main><script type="module">import{Client}from "https://cdn.jsdelivr.net/npm/@gradio/client@1.19.1/dist/index.min.js";const btn=document.getElementById("go"),status=document.getElementById("status"),log=document.getElementById("log"),video=document.getElementById("video");btn.onclick=async()=>{btn.disabled=true;try{status.textContent="Conectando con LTX Video ZeroGPU…";const app=await Client.connect("Lightricks/ltx-video-distilled",{events:["status","data"]});status.textContent="Enviando generación…";const job=app.submit("/text_to_video",["A cinematic realistic snowy mountain landscape at dawn, natural clouds moving across the peaks, subtle aerial camera push-in, documentary cinematography, no text, no logos.","worst quality, inconsistent motion, blurry, jittery, distorted, text, logos",null,null,256,256,"text-to-video",3,9,Math.floor(Math.random()*4294967295),true,3,false]);let output=null;for await(const msg of job){if(msg.type==="status"){status.textContent="LTX: "+(msg.stage||msg.status||"procesando")+(msg.position!=null?" · cola "+msg.position:"");log.textContent=JSON.stringify(msg,null,2)}if(msg.type==="data"){output=msg.data?.[0];}}const url=typeof output==="string"?output:(output?.url||output?.path||output?.video?.url||"");if(!url)throw new Error("LTX terminó sin devolver una URL de vídeo.");video.src=url;video.hidden=false;status.textContent="✓ Vídeo IA generado correctamente.";log.textContent=JSON.stringify({provider:"Hugging Face ZeroGPU · LTX Video",output},null,2)}catch(e){status.textContent="✕ Error";log.textContent=e?.stack||String(e)}finally{btn.disabled=false}};</script></body></html>`)});\napp.get('/api/video-ai-test/:jobId',async(req,res)=>{
