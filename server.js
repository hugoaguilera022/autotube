...
async function renderAutotubeVideo({scenes,mediaResults=[],aiClips=[],narrationAudio=[],musicBuffer=null,onProgress=()=>{},finalOutputPath}){
  ...
  try{
    const clips=[];
    const usableScenes=scenes.map((scene,i)=>({scene,found:mediaResults.find(x=>String(x.number)===String(scene.number))||mediaResults[i]||null,aiClip:aiClips[i]||null,audio:narrationAudio[i]||null})).filter(x=>x.aiClip?.buffer||x.found?.media?.find(m=>m.downloadUrl)?.downloadUrl);
    ...
    for(let i=0;i<total;i++){
      ...
      if(aiClip?.buffer){await fs.writeFile(input,aiClip.buffer)}else{await downloadToFile(media.downloadUrl,input)}
      let audioInput=null;
