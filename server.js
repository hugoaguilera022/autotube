...
async function analyzeDownloadedReferenceMedia(file,video){
  ...
  try{
    ...
    analysis.generationDirectives.preferredSceneCount=constantImage?1:Number(gd.preferredSceneCount||vp.estimatedSceneCount||sp.segmentCount||Math.max(1,Math.ceil(durationSeconds/20)));
    ...
    try{
      ...
      try{
        ...
        analysis={
          ...
          structureProfile:{opening:'strong thematic opening',pacing:'steady',transitions:'smooth',segmentCount:Math.max(1,Math.ceil(durationSeconds/20)),segmentDurations:[],visualContinuity:'thematic continuity',timestamps:[],sceneSegments:[]},
          generationDirectives:{useSingleContinuousVisual:false,preferredSceneCount:Math.max(1,Math.ceil(durationSeconds/20)),preserveVisualContinuity:true,preserveAudioContinuity:true,visualSearchStrategy:'search by exact reference topic and concrete subjects',musicStrategy:'original instrumental bed',narrationStrategy:'original narration when topic suggests speech',animationStrategy:'cinematic subtle motion'},
...
app.get('/api/ai/goalsave-api',async(_req,res)=>{try{const{Client}=require('@gradio/client');const app=await Client.connect('Goalsave/ltx-2-5-studio');const api=await app.view_api();res.json({ok:true,api});}catch(e){res.status(502).json({ok:false,error:String(e?.message||e)})}});
