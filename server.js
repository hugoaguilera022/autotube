...
app.get('/video-ai-test-browser',async(_req,res)=>{res.type('html').send('<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AutoTube · Prueba vídeo IA</title></head><body style="font-family:system-ui;max-width:700px;margin:40px auto;padding:20px"><h1>Prueba real de vídeo IA</h1><p>Generación directa en Hugging Face ZeroGPU, sin cargar Render.</p><button id="go">Generar clip de prueba</button><p id="status">Esperando…</p><pre id="log"></pre><video id="video" controls hidden style="width:100%"></video><script type="module">import{Client}from "https://cdn.jsdelivr.net/npm/@gradio/client@1.19.1/dist/index.min.js";const b=document.getElementById("go"),s=document.getElementById("status"),l=document.getElementById("log"),v=document.getElementById("video");b.onclick=async()=>{b.disabled=true;try{s.textContent="Conectando…";const a=await Client.connect("Lightricks/ltx-video-distilled");s.textContent="Generando…";const j=a.submit("/text_to_video",{prompt:"A cinematic realistic snowy mountain landscape at dawn, natural clouds moving, subtle aerial camera movement, realistic documentary cinematography, no text, no logos.",negative_prompt:"worst quality, blurry, jittery, distorted, text, logos",image_n:null,video_n:null,height:256,width:256,mode:"text-to-video",duration:3,frames_to_use:9,seed:Math.floor(Math.random()*4294967295),randomize_seed:true,guidance_scale:3,improve_texture:false});let o=null;for await(const m of j){if(m.type==="status")s.textContent="LTX: "+(m.stage||m.status||"procesando");if(m.type==="data")o=m.data?.[0]}const u=typeof o==="string"?o:(o?.url||o?.path||o?.video?.url||"");if(!u)throw Error("LTX no devolvió el vídeo.");v.src=u;v.hidden=false;s.textContent="✓ Vídeo IA generado.";l.textContent=JSON.stringify(o,null,2)}catch(e){s.textContent="✕ Error";l.textContent=e?.stack||String(e)}finally{b.disabled=false}};</script></body></html>')});
